...
bool DX11Hook::StartHook()
{
    ...
    if (!hooked)
    {
        ...
        BeginHook();
        HookFuncs(
            std::make_pair<void**, void*>(&(PVOID&)DX11Hook::Present, &DX11Hook::MyPresent),
...
HRESULT STDMETHODCALLTYPE DX11Hook::MyPresent(IDXGISwapChain* _this, UINT SyncInterval, UINT Flags)
{
    ...
    __except (EXCEPTION_EXECUTE_HANDLER) {
        ...
        PRINT_DEBUG("Exception in DX11 MyPresent overlay preparation - continuing with present\n");
    }
...
HRESULT STDMETHODCALLTYPE DX11Hook::MyResizeTarget(IDXGISwapChain* _this, const DXGI_MODE_DESC* pNewTargetParameters)
{
    ...
    __except (EXCEPTION_EXECUTE_HANDLER) {
        ...
        PRINT_DEBUG("Exception in DX11 MyResizeTarget ResetRenderState - continuing with resize\n");
    }
    
    return (_this->*DX11Hook::Instance()->ResizeTarget)(pNewTargetParameters);
}

HRESULT STDMETHODCALLTYPE DX11Hook::MyResizeBuffers(IDXGISwapChain* _this, UINT BufferCount, UINT Width, UINT Height, DXGI_FORMAT NewFormat, UINT SwapChainFlags)
{
    // Add null check for swap chain
    if (!_this) {
        return E_INVALIDARG;
    ...
    __except (EXCEPTION_EXECUTE_HANDLER) {
        // Handle any exceptions during reset
        PRINT_DEBUG("Exception in DX11 MyResizeBuffers ResetRenderState - continuing with resize\n");
    }
...
const char* DX11Hook::GetLibName() const
{
    return DX11_DLL;
}
