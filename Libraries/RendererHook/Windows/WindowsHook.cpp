...
bool WindowsHook::StartHook()
{
    ...
    if (!hooked)
    {
        ...
        BeginHook();
        HookFuncs(
            std::make_pair<void**, void*>(&(PVOID&)GetRawInputBuffer, &WindowsHook::MyGetRawInputBuffer),
            std::make_pair<void**, void*>(&(PVOID&)GetRawInputData, &WindowsHook::MyGetRawInputData),
            std::make_pair<void**, void*>(&(PVOID&)SetCursorPos, &WindowsHook::MySetCursorPos)
        );
        EndHook();
...
void WindowsHook::PrepareForOverlay(HWND hWnd)
{
    if (!initialized)
    {
        ...
        _game_hwnd = hWnd;
        _game_wndproc = (WNDPROC)SetWindowLongPtr(hWnd, GWLP_WNDPROC, (LONG_PTR)&WindowsHook::HookWndProc);
...
HWND WindowsHook::GetGameHwnd() const
{
    return _game_hwnd;
}

WNDPROC WindowsHook::GetGameWndProc() const
{
...
void RawMouseEvent(RAWINPUT& raw)
{
    if (raw.header.dwType == RIM_TYPEMOUSE)
    {
        if (raw.data.mouse.usButtonFlags & RI_MOUSE_LEFT_BUTTON_DOWN)
            ImGui_ImplWin32_WndProcHandler(WindowsHook::Instance()->GetGameHwnd(), WM_LBUTTONDOWN, 0, 0);
        else if (raw.data.mouse.usButtonFlags & RI_MOUSE_LEFT_BUTTON_UP)
            ImGui_ImplWin32_WndProcHandler(WindowsHook::Instance()->GetGameHwnd(), WM_LBUTTONUP, 0, 0);
        else if (raw.data.mouse.usButtonFlags & RI_MOUSE_MIDDLE_BUTTON_DOWN)
            ImGui_ImplWin32_WndProcHandler(WindowsHook::Instance()->GetGameHwnd(), WM_MBUTTONDOWN, 0, 0);
        else if (raw.data.mouse.usButtonFlags & RI_MOUSE_MIDDLE_BUTTON_UP)
            ImGui_ImplWin32_WndProcHandler(WindowsHook::Instance()->GetGameHwnd(), WM_MBUTTONUP, 0, 0);
        else if (raw.data.mouse.usButtonFlags & RI_MOUSE_RIGHT_BUTTON_DOWN)
            ImGui_ImplWin32_WndProcHandler(WindowsHook::Instance()->GetGameHwnd(), WM_RBUTTONDOWN, 0, 0);
        else if (raw.data.mouse.usButtonFlags & RI_MOUSE_RIGHT_BUTTON_UP)
            ImGui_ImplWin32_WndProcHandler(WindowsHook::Instance()->GetGameHwnd(), WM_RBUTTONUP, 0, 0);
    }
}

LRESULT CALLBACK WindowsHook::HookWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    return CallWindowProc(WindowsHook::Instance()->_game_wndproc, hWnd, uMsg, wParam, lParam);
}

UINT WINAPI WindowsHook::MyGetRawInputBuffer(PRAWINPUT pData, PUINT pcbSize, UINT cbSizeHeader)
{
    if (pData == nullptr || !OverlayBase::Instance->ShowOverlay())
        return WindowsHook::Instance()->GetRawInputBuffer(pData, pcbSize, cbSizeHeader);

    int num = WindowsHook::Instance()->GetRawInputBuffer(pData, pcbSize, cbSizeHeader);
    for (int i = 0; i < num; ++i)
...
UINT WINAPI WindowsHook::MyGetRawInputData(HRAWINPUT hRawInput, UINT uiCommand, LPVOID pData, PUINT pcbSize, UINT cbSizeHeader)
{
    if (pData == nullptr || !OverlayBase::Instance->ShowOverlay())
        return WindowsHook::Instance()->GetRawInputData(hRawInput, uiCommand, pData, pcbSize, cbSizeHeader);

    WindowsHook::Instance()->GetRawInputData(hRawInput, uiCommand, pData, pcbSize, cbSizeHeader);
...
const char* WindowsHook::GetLibName() const
{
    return WINDOWS_DLL;
}