...
bool DX9Hook::StartHook()
{
	if (!hooked)
	{
		...
		BeginHook();
		HookFuncs(
			std::make_pair<void**, void*>(&(PVOID&)Reset, &DX9Hook::MyReset),
			std::make_pair<void**, void*>(&(PVOID&)Present, &DX9Hook::MyPresent)
		);
		if (PresentEx != nullptr)
		{
			HookFuncs(
				std::make_pair<void**, void*>(&(PVOID&)PresentEx, &DX9Hook::MyPresentEx)
				//std::make_pair<void**, void*>(&(PVOID&)EndScene, &DX9Hook::MyEndScene)
			);
		}
		EndHook();
...
HRESULT STDMETHODCALLTYPE DX9Hook::MyPresent(IDirect3DDevice9* _this, CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
{
...
HRESULT STDMETHODCALLTYPE DX9Hook::MyPresentEx(IDirect3DDevice9Ex* _this, CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion, DWORD dwFlags)
{
	DX9Hook::Instance()->uses_present = true;
	DX9Hook::Instance()->PrepareForOverlay(_this);
...
const char* DX9Hook::GetLibName() const
{
	return DX9_DLL;
}
