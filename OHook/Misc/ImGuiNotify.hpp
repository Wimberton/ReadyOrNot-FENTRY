...
#define NOTIFY_FADE_IN_OUT_TIME	                    120	                    // Fade in and out duration
#define NOTIFY_DEFAULT_DISMISS                      3000		            // Auto dismiss after X ms (default, applied only of no data provided in constructors)
#define NOTIFY_OPACITY                              1.0f		            // 0-1 Toast opacity
#define NOTIFY_RENDER_LIMIT                         6                       // Max number of toasts rendered at the same time. Set to 0 for unlimited
// Bools
...
#define NOTIFY_USE_PROGRESS_BAR                     true                    // If true, a duration progress bar will be rendered at the bottom of the toast.
// Other
#define NOTIFY_TOAST_POS                            ImGuiToastPos::TopLeft  // Toast positioning on screen
...
public:
    ...
    /**
     * @brief Get the dismiss time of the toast notification.
     *
     * @return int The dismiss time in milliseconds.
     */
    inline int getDismissTime()
    {
...
namespace ImGui
{
    ...
    inline void RenderNotifications()
    {
        ...
        for (size_t i = 0; i < notifications.size(); ++i)
        {
            ...
            switch (NOTIFY_TOAST_POS) {
            case ImGuiToastPos::TopLeft:
                toastPos = ImVec2(NOTIFY_PADDING_X, NOTIFY_PADDING_Y + height);
                pivot = ImVec2(0.0f, 0.0f);
                break;
            case ImGuiToastPos::TopCenter:
                toastPos = ImVec2(mainWindowPos.x / 2, NOTIFY_PADDING_Y + height);
                pivot = ImVec2(0.5f, 0.0f);
                break;
            case ImGuiToastPos::TopRight:
                toastPos = ImVec2(mainWindowPos.x - NOTIFY_PADDING_X, NOTIFY_PADDING_Y + height);
                pivot = ImVec2(1.0f, 0.0f);
                break;
            case ImGuiToastPos::BottomLeft:
                toastPos = ImVec2(NOTIFY_PADDING_X, mainWindowPos.y - NOTIFY_PADDING_Y - height);
                pivot = ImVec2(0.0f, 1.0f);
                break;
            case ImGuiToastPos::BottomCenter:
                toastPos = ImVec2(mainWindowPos.x / 2, mainWindowPos.y - NOTIFY_PADDING_Y - height);
                pivot = ImVec2(0.5f, 1.0f);
                break;
            case ImGuiToastPos::BottomRight:
                toastPos = ImVec2(mainWindowPos.x - NOTIFY_PADDING_X, mainWindowPos.y - NOTIFY_PADDING_Y - height);
                pivot = ImVec2(1.0f, 1.0f);
                break;
            case ImGuiToastPos::Center:
                toastPos = ImVec2(mainWindowPos.x / 2, mainWindowPos.y / 2 - height);
            ...
            SetNextWindowPos(toastPos, ImGuiCond_Always, pivot);
#endif
            ...
            const ImVec2 textSize = CalcTextSize(content);
            const ImVec2 maxSize = ImVec2(mainWindowSize.x / 3.f, textSize.y + NOTIFY_PADDING_Y * 2);
            SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(maxSize.x, maxSize.y * 2));
            ...
            // Here we render the toast content
            {
                ...
                if (NOTIFY_USE_PROGRESS_BAR)
                {
                    const float totalDuration = NOTIFY_FADE_IN_OUT_TIME + currentToast->getDismissTime() + NOTIFY_FADE_IN_OUT_TIME;
                    const float elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(currentToast->getElapsedTime()).count();
                    const float progress = elapsedTime / totalDuration;
                    ProgressBar(progress, ImVec2(GetWindowSize().x - NOTIFY_PADDING_X, 1.0f));
                }
